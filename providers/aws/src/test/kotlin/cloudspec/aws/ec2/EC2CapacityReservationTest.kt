...
class EC2CapacityReservationTest {
    ...
    fun `should convert SDK model to CloudSpec model`() {
        ...
        assertEquals(source.instancePlatformAsString(), target.instancePlatform)
        assertEquals(source.availabilityZone(), target.availabilityZone)
        assertEquals(source.tenancyAsString(), target.tenancy)
        ...
        assertEquals(source.stateAsString(), target.state)
        assertEquals(source.endDate(), target.endDate)
        assertEquals(source.endDateTypeAsString(), target.endDateType)
        assertEquals(source.instanceMatchCriteriaAsString(), target.instanceMatchCriteria)
